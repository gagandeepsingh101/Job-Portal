...
export default function AdminJobEdit() {
  ...
  const fetchJob = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrors({ api: data.error || 'Failed to fetch job.' })
      }
    } catch {
      setErrors({ api: 'Failed to fetch job.' })
    }
    setLoading(false)
  }
  useEffect(() => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccess(true)
        toast.success(data.message || "New job update successfully")
        router.push('/admin/dashboard')
      } else {
        const data = await res.json()
        setErrors({ api: data.error || 'Failed to update job.' })
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-8">
      <div className="bg-white shadow-lg rounded-lg p-8 border border-gray-100">
        <h1 className="text-3xl font-bold mb-8 text-blue-700">Edit Job</h1>
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {success && <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-2 rounded mb-2 text-sm">Job updated successfully!</div>}
